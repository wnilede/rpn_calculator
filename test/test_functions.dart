...
@isTest
void testStrategyIsConsistent<TParameters extends OptimizationParameters<TParameters>>(String description, OptimizationStrategy<TParameters> strategy, List<List<TParameters>> scenarios, List<Size> sizes) {
  test(
    description,
    () {
      for (List<TParameters> parametersChildren in scenarios) {
        ...
        for (Size size in sizes) {
          ...
          expect(
            ...
            isTrue,
            reason: 'The values returned by getOptimalWidths are not all non-negative.$positionDescriber',
          );
          expect(
